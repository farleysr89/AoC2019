...
namespace AdventCode3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var lines1 = ProcessWire(wire1);
            var lines2 = ProcessWire(wire2);

            var minDist = int.MaxValue;
            //int checks = 0;
            foreach (var line1 in lines1.FindAll(x => x.horizontal))
            {
                foreach (var line2 in lines2.FindAll(x => !x.horizontal))
                {
                    //checks += 1;
                    if (CheckLines(line1, line2))
                    {
                        Intersection intersection = GetIntersection(line1, line2,lines1,lines2);
            ...
            //Console.WriteLine((lines1.ToArray()[3].ToString(), lines2.ToArray()[3].ToString()));
            //Console.WriteLine(CheckLines(lines1.ToArray()[3], lines2.ToArray()[3]));
            //Console.WriteLine(GetIntersection(lines1.ToArray()[3], lines2.ToArray()[3]));
        }

        static List<Line> ProcessWire(string wire)
        {
            int x = 0;
            int y = 0;
            var lines = new List<Line> { };
            int count = 0;
            foreach (string move in wire.Split(","))
            {
                var line = ProcessMove(move, x, y, count++);
                lines.Add(line);
                x = line.b.x;
                y = line.b.y;
            }
        ...
        static Line ProcessMove(string move, int x, int y, int count)
        {
            if (move[0] == 'U')
                return new Line(new Point(x, y), new Point(x, y + Int32.Parse(move.Substring(1))), false, count);
            if (move[0] == 'D')
                return new Line(new Point(x, y), new Point(x, y - Int32.Parse(move.Substring(1))), false, count);
            if (move[0] == 'L')
                return new Line(new Point(x, y), new Point(x - Int32.Parse(move.Substring(1)), y), true, count);
            if (move[0] == 'R')
                return new Line(new Point(x, y), new Point(x + Int32.Parse(move.Substring(1)), y), true, count);
            return new Line(new Point(0, 0), new Point(0, 0), false, 0);
        }
        ...
        static bool CheckLines(Line a, Line b)
        {
            if (a.horizontal)
                return Check(a.a.x, a.b.x, b.a.x)
                    && Check(b.a.y, b.b.y, a.a.y);
            return Check(b.a.x, b.b.x, a.a.x)
                && Check(a.a.y, a.b.y, b.a.y);
        }

        static Intersection GetIntersection(Line a, Line b, List<Line> lines1, List<Line> lines2)
        {
            Point intersection;
            if (!a.horizontal)
                intersection = new Point(a.a.x, b.a.y);
            else
                intersection = new Point(b.a.x, a.a.y);
            return new Intersection(intersection.x, intersection.y, GetDistance(a, b, lines1, lines2));

        }

        static int GetDistance(Line a, Line b, List<Line> lines1, List<Line> lines2)
        {
            int total = 0;
            foreach(Line line in lines1.FindAll(x => x.index < a.index))
                total += line.getLength();
            foreach(Line line in lines2.FindAll(x => x.index < b.index))
                total += line.getLength();
            total += a.horizontal ? Math.Abs(b.a.x - a.a.x) + Math.Abs(a.a.y - b.a.y) : Math.Abs(a.a.x - b.a.x) + Math.Abs(b.a.y - a.a.y);
            return total;
    ...
    class Line
    {
        ...
        public int index;
        public Line(Point a, Point b, bool horizontal, int index)
        {
            this.a = a;
            this.b = b;
